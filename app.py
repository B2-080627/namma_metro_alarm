...
def load_data():
    ...
    # Bengaluru bounding box: lat 12.8–13.2, lon 77.4–77.8
    # If values look swapped, flip them.
    lat_ok = df["latitude"].between(12.0, 14.0).all()
    lon_ok = df["longitude"].between(77.0, 78.0).all()

    if not lat_ok and not lon_ok:
        # Both wrong — try swapping
        df = df.rename(columns={"latitude": "longitude", "longitude": "latitude"})
    elif not lat_ok:
        # Latitude column holds longitude values — swap
...
# Build stations JSON — explicitly order lat then lon to avoid any column confusion
stations_records = df[["Name","latitude","longitude"]].copy()
stations_records.columns = ["name","lat","lon"]

# Clamp to valid Bengaluru range as a final safeguard
stations_records = stations_records[
    stations_records["lat"].between(12.0, 14.0) &
    stations_records["lon"].between(77.0, 78.5)
]

stations_json = stations_records.to_json(orient="records")

# Use a non-f-string for the JS tile URL placeholders to avoid brace conflicts
...
/* Debug panel */
#debugPanel{{
  position:absolute;bottom:48px;right:8px;z-index:1000;
  background:rgba(15,17,23,.85);border:1px solid #334155;border-radius:8px;
  padding:8px 12px;font-size:.68rem;color:#94a3b8;max-width:220px;display:none;
}}
...
<div class="topbar">
  ...
  <div style="display:flex;gap:7px;align-items:center;">
    ...
    <button class="btn btn-sm" onclick="toggleDebug()" title="Debug coordinates">🐛</button>
...
<div id="debugPanel">
  <b style="color:#4ade80">Coordinate Debug</b><br>
  <span id="dbgInfo">Hover a station marker to inspect its coordinates.</span>
</div>
...
<script>
// ── STATION DATA (validated by Python before injection) ──
...
// ── Validate stations client-side too: lat must be ~12-14, lon ~77-78 ──
const VALID_STATIONS = STATIONS.filter(s => {{
  const latOk = s.lat >= 12.0 && s.lat <= 14.0;
  const lonOk = s.lon >= 77.0 && s.lon <= 78.5;
  if (!latOk || !lonOk) {{
    console.warn('Skipping station with suspicious coords:', s.name, s.lat, s.lon);
  }}
  return latOk && lonOk;
}});

console.log('Total stations:', STATIONS.length, '| Valid:', VALID_STATIONS.length);
if (VALID_STATIONS.length === 0) {{
  console.error('No valid stations found! Check CSV column order (latitude vs longitude).');
}}

// ── MAP — tile URLs passed as JS string literals (no f-string brace issues) ──
const TILE_URL = "{TILE_URL}";
const SAT_URL  = "{SAT_URL}";

const map = L.map('map', {{zoomControl:true}}).setView([12.9716, 77.5946], 12);
const streetLayer = L.tileLayer(TILE_URL, {{attribution:'© OpenStreetMap', maxZoom:19}}).addTo(map);
const satLayer    = L.tileLayer(SAT_URL,  {{attribution:'Esri', maxZoom:19}});
L.control.layers({{'Street': streetLayer}}, {{'Satellite': satLayer}}).addTo(map);

// Plot station markers using VALID_STATIONS only
VALID_STATIONS.forEach(s => {{
  const marker = L.circleMarker([s.lat, s.lon], {{
    radius: 5, color: '#60a5fa', fillColor: '#3b82f6', fillOpacity: .8, weight: 1.5
  }}).addTo(map);

  // Rich tooltip showing exact coordinates
  marker.bindTooltip(
    `<b>${{s.name}}</b><br><span style="font-size:.75rem;color:#94a3b8">${{s.lat.toFixed(5)}}, ${{s.lon.toFixed(5)}}</span>`,
    {{sticky: true}}
  );

  marker.on('mouseover', () => {{
    document.getElementById('dbgInfo').innerHTML =
      `<b>${{s.name}}</b><br>lat: ${{s.lat.toFixed(6)}}<br>lon: ${{s.lon.toFixed(6)}}`;
  }});
}});
...
function onSearch() {{
  ...
  filtered = VALID_STATIONS.filter(s => s.name.toLowerCase().includes(q));
...
function renderDropdown(list, q) {{
  ...
  dd.innerHTML = list.slice(0,12).map((s,i) => {{
    ...
    const hi  = s.name.replace(re,'<mark>$1</mark>');
...
function nearestStation(lat, lon) {{
  ...
  VALID_STATIONS.forEach(s => {{ const d=haversineM(lat,lon,s.lat,s.lon); if(d<bestD){{bestD=d;best=s;}} }});
  return best;
}}
...
function startKeepAlive() {{
  noSleep.enable();
  kaSeconds = 0;
  const badge=document.getElementById('kaBadge'), iKa=document.getElementById('iKa');
  kaTimer = setInterval(() => {{
    kaSeconds += 25;
    const m=Math.floor(kaSeconds/60), s=kaSeconds%60;
    const txt = m ? m+'m '+s+'s' : s+'s';
    badge.textContent='🟢 '+txt; badge.className='ka live'; iKa.textContent=txt;
    fetch('data:text/plain,ka').catch(()=>{{}});
  }}, 25000);
}}
function stopKeepAlive() {{
  noSleep.disable(); clearInterval(kaTimer); kaTimer=null;
...
function onPosition(pos) {{
  ...
  const banner=document.getElementById('alarmBanner'), audio=document.getElementById('alarmAudio');
  if (distM!==null) {{
    if (distM<=ALARM_DIST) {{
